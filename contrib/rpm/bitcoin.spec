...
mv %{buildroot}%{_bindir}/bitcoinvoted %{buildroot}%{_sbindir}/bitcoinvoted

# systemd stuff
mkdir -p %{buildroot}%{_tmpfilesdir}
cat <<EOF > %{buildroot}%{_tmpfilesdir}/bitcoinvote.conf
d /run/bitcoind 0750 bitcoin bitcoin -
EOF
touch -a -m -t 201504280000 %{buildroot}%{_tmpfilesdir}/bitcoinvote.conf
...
CONFIG_FILE="%{_sysconfdir}/bitcoin/bitcoinvote.conf"
DATA_DIR="%{_localstatedir}/lib/bitcoinvote"
PID_FILE="/run/bitcoinvoted/bitcoinvoted.pid"
EOF
touch -a -m -t 201504280000 %{buildroot}%{_sysconfdir}/sysconfig/bitcoinvote

mkdir -p %{buildroot}%{_unitdir}
cat <<EOF > %{buildroot}%{_unitdir}/bitcoinvote.service
[Unit]
...
[Service]
Type=forking
ExecStart=%{_sbindir}/bitcoinvoted -daemon -conf=\${CONFIG_FILE} -datadir=\${DATA_DIR} -pid=\${PID_FILE} \$OPTIONS
EnvironmentFile=%{_sysconfdir}/sysconfig/bitcoinvote
User=bitcoinvote
Group=bitcoinvote
...
EOF
touch -a -m -t 201504280000 %{buildroot}%{_unitdir}/bitcoinvote.service
#end systemd stuff

mkdir %{buildroot}%{_sysconfdir}/bitcoinvote
mkdir -p %{buildroot}%{_localstatedir}/lib/bitcoinvote

#SELinux
for selinuxvariant in %{selinux_variants}; do
	install -d %{buildroot}%{_datadir}/selinux/${selinuxvariant}
	install -p -m 644 SELinux/bitcoinvote.pp.${selinuxvariant} %{buildroot}%{_datadir}/selinux/${selinuxvariant}/bitcoinvote.pp
done
...
cat <<EOF > %{buildroot}%{_datadir}/applications/bitcoinvote-core.desktop
[Desktop Entry]
Encoding=UTF-8
Name=BitcoinVote
Comment=BitcoinVote P2P Cryptocurrency
Comment[fr]=BitcoinVote, monnaie virtuelle cryptographique pair à pair
Comment[tr]=BitcoinVote, eşten eşe kriptografik sanal para birimi
Exec=bitcoin-qt %u
Terminal=false
Type=Application
Icon=bitcoin128
MimeType=x-scheme-handler/bitcoinvote;
Categories=Office;Finance;
EOF
# change touch date when modifying desktop
touch -a -m -t 201511100546 %{buildroot}%{_datadir}/applications/bitcoinvote-core.desktop
%{_bindir}/desktop-file-validate %{buildroot}%{_datadir}/applications/bitcoinvote-core.desktop

# KDE protocol - change the touch timestamp if modifying
mkdir -p %{buildroot}%{_datadir}/kde4/services
cat <<EOF > %{buildroot}%{_datadir}/kde4/services/bitcoinvote-core.protocol
[Protocol]
exec=bitcoinvote-qt '%u'
protocol=bitcoinvote
input=none
output=none
helper=true
listing=
...
%pre server
getent group bitcoinvote >/dev/null || groupadd -r bitcoinvote
getent passwd bitcoinvote >/dev/null ||
	useradd -r -g bitcoinvote -d /var/lib/bitcoinvote -s /sbin/nologin \
	-c "Bitcoin wallet server" bitcoinvote
exit 0
...
for selinuxvariant in %{selinux_variants}; do
	%{_sbindir}/semodule -s ${selinuxvariant} -i %{_datadir}/selinux/${selinuxvariant}/bitcoinvote.pp &> /dev/null || :
done
...
%{_sbindir}/restorecon -R %{_localstatedir}/lib/bitcoinvote || :
fi
...
%preun server
%systemd_preun bitcoinvote.service

%postun server
%systemd_postun bitcoinvote.service
# SELinux
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	...
	%{_sbindir}/fixfiles -R bitcoinvote-server restore &> /dev/null || :
	[ -d %{_localstatedir}/lib/bitcoin ] && \
		%{_sbindir}/restorecon -R %{_localstatedir}/lib/bitcoinvote &> /dev/null || :
	fi
fi
...
%attr(0755,root,root) %{_bindir}/bitcoinvote-qt
%attr(0644,root,root) %{_datadir}/applications/bitcoinvote-core.desktop
%attr(0644,root,root) %{_datadir}/kde4/services/bitcoinvote-core.protocol
...
%attr(0755,root,root) %{_sbindir}/bitcoinvoted
%attr(0644,root,root) %{_tmpfilesdir}/bitcoinvote.conf
%attr(0644,root,root) %{_unitdir}/bitcoinvote.service
%dir %attr(0750,bitcoin,bitcoin) %{_sysconfdir}/bitcoinvote
%dir %attr(0750,bitcoin,bitcoin) %{_localstatedir}/lib/bitcoinvote
%config(noreplace) %attr(0600,root,root) %{_sysconfdir}/sysconfig/bitcoinvote
...
%attr(0755,root,root) %{_bindir}/bitcoinvote-cli
%attr(0755,root,root) %{_bindir}/bitcoinvote-tx
